...
# error: no agent named developer-beta

# 3. developer-alpha is alive: has it done the work? Inspect before re-sending anything.
python3 scripts/tm.py report developer-alpha --lines 300
python3 scripts/tm.py diff --cwd ~/code/acme --stat
```

If `developer-alpha` already edited `acme/ops.py`, resume it — never resend the brief.
`developer-beta` is orphaned, so it has no live state to resume; check whether its README
...
python3 scripts/tm.py spawn --cwd ~/code/acme --project acme --name developer-gamma --task tsk_2222
...
EOF
python3 scripts/tm.py send developer-gamma --brief "<printed-path>"
```
