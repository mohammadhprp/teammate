...
**One prompt** — paste this into your AI agent to install and get an intro:
...
`opencode` · `codex` · `claude` (Claude Code, installed as a plugin) · `pi` ·
`omp`. The harness is auto-detected; pin one with `--harness`. See the
[Install guide](docs/INSTALL.md) for options and details.
